...
import net.wyun.list.bean.Folder;
import net.wyun.list.bean.MimeTypeMapping;
import net.wyun.list.bean.VersionBean;
import net.wyun.list.comparator.*;
import net.wyun.list.validator.NameValidator;
...
@Named("documentsData")
@SessionScoped
public class DocumentsDataHelper {

	private static final Log log = ExoLogger.getLogger(DocumentsDataHelper.class
			.getName());
	...
	@Inject
	public DocumentsDataHelper(RepositoryService repositoryService,
			SessionProviderService sessionProviderService,
			NodeHierarchyCreator nodeHierarchyCreator,
			NewFolksonomyService newFolksonomyService, LinkManager linkManager) {
	...
	public SessionProvider getUserSessionProvider() {
		SessionProvider sessionProvider = sessionProviderService_
	...
	protected void initNodetypes() {
		...
		try {
			...
			try {
				...
				namespaceRegistry.registerNamespace("adn",
						"http://www.exoplatform.com/jcr/adn/1.0");
			}

			ExtendedNodeTypeManager nodeTypeManager = (ExtendedNodeTypeManager) session
			...
			try {
				...
				nodeTypeManager.registerNodeType(adnMeta,
	...
	protected boolean createNodeIfNotExist(String filter, String name) {
		...
		try {
			...
			String space = getSpaceName();
			String path = (space != null) ? getSpacePath(space)
					: getUserPrivatePath();
			...
			if (!rootNode.hasNode(path + "/" + filter  + name)) {
				...
				SessionProvider systemProvider = SessionProvider
						.createSystemProvider();
				Session sysSession = systemProvider.getSession("collaboration",
						repositoryService_.getCurrentRepository());
				try {
	...
	protected Folder getNodes(String filter) {
		...
		try {
			...
			if (filter.startsWith("Folksonomy/")) {
				...
				Node parentNode = (space != null) ? rootNode.getNode(path
						+ "/ApplicationData/Tags/") : rootNode.getNode(path
						+ "/Folksonomy/");
				docNode = newFolksonomyService_.getDataDistributionType()
						.getDataNode(parentNode, tag);
			} else {
			...
			if (docNode.hasProperty(TIMESTAMP_PROPERTY)) {
				folder.setTimestamp(docNode.getProperty(TIMESTAMP_PROPERTY)
						.getLong());
			} else {
	...
	private File getFileFromNode(Node node, String space, boolean full)
			throws Exception {
		File file = new File();
		// set name
		...
		// set public url
		HttpServletRequest request = Util.getPortalRequestContext()
				.getRequest();
		String baseURI = request.getScheme() + "://" + request.getServerName()
				+ ":" + String.format("%s", request.getServerPort());

		String url = baseURI + "/documents/file/"
				+ Util.getPortalRequestContext().getRemoteUser() + "/"
				+ file.getUuid() + "/" + file.getName();
		file.setPublicUrl(url);

		// set tags
		List<Node> tags;
		if (space != null) {
			tags = newFolksonomyService_.getLinkedTagsOfDocumentByScope(
					NewFolksonomyService.GROUP, "/spaces/" + space, node,
					"collaboration");
		} else {
			tags = newFolksonomyService_.getLinkedTagsOfDocumentByScope(
					NewFolksonomyService.PRIVATE, Util
							.getPortalRequestContext().getRemoteUser(), node,
					"collaboration");
		}
		...
		if (full) {
			...
			if (node.isNodeType("mix:versionable")) {
				VersionIterator iterator = node.getVersionHistory()
						.getAllVersions();
				...
				while (iterator.hasNext()) {
					...
					// " :: " + version.getCreated().getTimeInMillis());
	...
	protected void renameFile(String id, String name) throws Exception {
		...
		Node node = getNodeById(id, session);
		String extension = (!id.contains("/")) ? node.getName().substring(
				node.getName().lastIndexOf(".")) : "";
		updateTimestamp(node);
		updateTimestamp(node.getParent());

		StringBuilder newPath = new StringBuilder(node.getParent().getPath())
				.append('/').append(name).append(extension);
	...
	protected void editTags(String uuid, String tags) throws Exception {
		...
		try {
			...
			/**
			 * TODO : Optimize this, remove existing if not in new list, add
			 * only if new
			 * */
			List<Node> tagsNodes;
			String space = getSpaceName();
			if (space != null) {
				tagsNodes = newFolksonomyService_
						.getLinkedTagsOfDocumentByScope(
								NewFolksonomyService.GROUP, "/spaces/" + space,
								node, "collaboration");
			} else {
				tagsNodes = newFolksonomyService_
						.getLinkedTagsOfDocumentByScope(
								NewFolksonomyService.PRIVATE, Util
										...
										.getRemoteUser(), node, "collaboration");
			}
			if (tagsNodes != null && tagsNodes.size() > 0) {
				for (Node tag : tagsNodes) {
					newFolksonomyService_.removeTagOfDocument(tag.getPath(),
							node, "collaboration");
				}
			}

			if (tags != null && !"".equals(tags)) {
				String[] atags = tags.replaceAll(" ", "").toLowerCase()
						.split(",");
				if (space != null) {
					newFolksonomyService_.addGroupsTag(atags, node,
							"collaboration",
							new String[] { "/spaces/" + space });
				} else {
					newFolksonomyService_.addPrivateTag(atags, node,
							"collaboration", Util.getPortalRequestContext()
									.getRemoteUser());
				}
	...
	protected void storeFile(String path, FileItem item, String name,
	...
	protected String storeFile(String path, FileItem item, String name,
		...
		try {
			...
			if (isPrivateContext) {
				Node userNode = nodeHierarchyCreator_.getUserNode(
						sessionProvider, name);
				homeNode = userNode.getNode("Private");
			} else {
				Node rootNode = session.getRootNode();
				homeNode = rootNode.getNode(getSpacePath(name)); //groups/spaces/qyspl
			}
			...
			if (!docNode.hasNode(filename)
				...
				jcrContent.setProperty("jcr:data", item.getInputStream());
				DocumentsDataHelper.updateTimestamp(fileNode);
				DocumentsDataHelper.updateTimestamp(docNode);
				DocumentsDataHelper.updateTimestamp(docNode.getParent());
				session.save();
			}
	...
	private String getUserPrivatePath() {
		...
		SessionProvider sessionProvider = getUserSessionProvider();
		try {
			Node userNode = nodeHierarchyCreator_.getUserNode(sessionProvider,
					userName);
	...
	private static void checkIfHasMetaType(Node node)
	...
	private boolean isAcceptedFile(String filename) {
		if (filename.endsWith(".jpg") || filename.endsWith(".png")
				|| filename.endsWith(".pdf") || filename.endsWith(".ppt")
				|| filename.endsWith(".xls") || filename.endsWith(".doc")
				|| filename.endsWith(".pptx") || filename.endsWith(".xlsx")
				|| filename.endsWith(".docx") || filename.endsWith(".odt")
