...
public class FileUploadUnmarshaller extends EntityUnmarshaller {
  ...
  public void unmarshall(
    ...
    try {
      ...
      HashMap<String, List<FileItem>> files = new HashMap<String, List<FileItem>>();
        ...
        if (file.isFormField()) {
          ...
          if (parameterArg == null) {
        	...
        	if(files.containsKey(name)){
        		...
        		List<FileItem> l = new ArrayList<FileItem>();
        		l.add(file);
        		files.put(name, l);
        	}
      ...
      for (Map.Entry<ContextualParameter, Object> argument : contextualArguments) {
        ...
        List<FileItem> file = files.get(contextualParam.getName());
        if (file != null && List.class.isAssignableFrom(contextualParam.getType())) {
